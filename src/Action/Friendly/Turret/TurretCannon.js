...
var TURRET_CANNON_PATROL_SPEED = [30, 30, 30, 30, 30];
var TURRET_CANNON_ROTATE_SPEED = [150, 160, 170, 180, 200];
var TURRET_CANNON_RANGE = [9, 10, 11, 12, 13];
var TURRET_CANNON_RECOIL = [6, 5, 4, 3, 2];
var TURRET_CANNON_COOLDOWN = [2, 1.8, 1.6, 1.4, 1.2];

// Do nothing, just for display purpose
var TURRET_CANNON_ACCURACY = [40, 44, 48, 52, 56];

function TurretCannon (battle, layer, x, y) {
	var SHADOW_OFFSET = 10;
	var PATROL_STAY_TIME = 5;
	
	// Properties
	this.m_active = true;
	this.m_type = TURRET_CANNON;
	
	this.m_x = x;
	this.m_y = y;
	this.m_level = 0;
	this.m_price = TURRET_CANNON_PRICE[0];
	...
	this.m_baseSprite = g_spritePool.GetSpriteFromPool("res/GSAction/Turret/2-Cannon/Base.png");
	...
	this.m_shadowSprite = g_spritePool.GetSpriteFromPool("res/GSAction/Turret/2-Cannon/Shadow.png");
	...
	this.m_turretSprite = g_spritePool.GetSpriteFromPool("res/GSAction/Turret/2-Cannon/Turret.png");
	...
	// Display info
	this.m_designPath = "res/GSAction/Turret/2-Cannon/Design.png";
	this.m_name = TURRET_CANNON_NAME;
	this.m_damageBar = [6, 6, 7, 7, 7];
	this.m_fireRateBar = [5, 5, 5, 6, 6];
	this.m_piercingBar = [2, 2, 3, 3, 4];
	this.m_rangeBar = [5, 6, 6, 6, 6];
	this.m_aoeBar = [3, 3, 3, 4, 4];
	
	this.MoneyRequireForNextLevel = function() {
		if (this.m_level < TURRET_CANNON_PRICE.length - 1) {
			return TURRET_CANNON_PRICE[this.m_level + 1];
		}
		return null;
	}
		...
		if (this.m_active == true) {
			if (this.m_stunned <= 0) {
				...
				if (this.m_target == null) {
					rotateAmount = TURRET_CANNON_PATROL_SPEED[this.m_level] * deltaTime;
	...
	this.Shoot = function() {
		if (cooldownCount <= 0) {
			...
			battle.SpawnProjectile (PROJECTILE_CANNON, this.m_x, this.m_y, angle, this);
	...
	this.GetRange = function() {
		if (this.m_blinded > 0) {
			return TURRET_CANNON_RANGE[this.m_level] * BLIND_RANGE_MULTIPLIER;
		}
		else {
			return TURRET_CANNON_RANGE[this.m_level];
		}
	}
	this.GetRecoil = function() {
		if (this.m_blinded > 0) {
			return TURRET_CANNON_RECOIL[this.m_level] * BLIND_RECOIL_MULTIPLIER;
		}
		else {
			return TURRET_CANNON_RECOIL[this.m_level];
		}
	}
	this.GetRotateSpeed = function() {
		if (this.m_slowed > 0) {
			return TURRET_CANNON_ROTATE_SPEED[this.m_level] * SLOW_ROTATE_MULTIPLIER;
		}
		else {
			return TURRET_CANNON_ROTATE_SPEED[this.m_level];
		}
	}
	this.GetCooldown = function() {
		if (this.m_slowed > 0) {
			return TURRET_CANNON_COOLDOWN[this.m_level] * SLOW_COOLDOWN_MULTIPLIER;
		}
		else {
			return TURRET_CANNON_COOLDOWN[this.m_level];
		}
	}
