var TURRET_GATLING_NAME = "Gatling gun";
var BLIND_RANGE_MULTIPLIER = 0.5;
var BLIND_RECOIL_MULTIPLIER = 3;
var SLOW_ROTATE_MULTIPLIER = 0.3;
var SLOW_COOLDOWN_MULTIPLIER = 2;

var TURRET_GATLING = 1;
var TURRET_GATLING_PRICE = [100, 55, 70, 85, 100];
var TURRET_GATLING_PATROL_SPEED = [30, 30, 30, 30, 30];
var TURRET_GATLING_ROTATE_SPEED = [250, 250, 250, 250, 250];
var TURRET_GATLING_RANGE = [6, 7, 8, 9, 10];
var TURRET_GATLING_RECOIL = [5, 4, 3, 3, 3];
var TURRET_GATLING_COOLDOWN = [0.10, 0.09, 0.08, 0.07, 0.06];

// Do nothing, just for display purpose
var TURRET_GATLING_ACCURACY = [60, 65, 70, 75, 80];

function TurretGatling (battle, layer, x, y) {
	var SHADOW_OFFSET = 10;
	var PATROL_STAY_TIME = 5;
	
	// Properties
	this.m_active = true;
	this.m_type = TURRET_GATLING;
	
	this.m_x = x;
	this.m_y = y;
	this.m_level = 0;
	this.m_price = TURRET_GATLING_PRICE[0];
	...
	this.m_baseSprite = g_spritePool.GetSpriteFromPool("res/GSAction/Turret/1-Gatling/Base.png");
	...
	this.m_shadowSprite = g_spritePool.GetSpriteFromPool("res/GSAction/Turret/1-Gatling/Shadow.png");
	...
	this.m_turretSprite = g_spritePool.GetSpriteFromPool("res/GSAction/Turret/1-Gatling/Turret.png");
	...
	// Display info
	this.m_designPath = "res/GSAction/Turret/1-Gatling/Design.png";
	this.m_name = TURRET_GATLING_NAME;
	this.m_damageBar = [2, 2, 2, 3, 3];
	this.m_fireRateBar = [6, 7, 8, 8, 9];
	this.m_piercingBar = [1, 1, 1, 2, 2];
	this.m_rangeBar = [4, 4, 4, 5, 5];
	this.m_aoeBar = [1, 1, 1, 1, 1];
	
	this.MoneyRequireForNextLevel = function() {
		if (this.m_level < TURRET_GATLING_PRICE.length - 1) {
			return TURRET_GATLING_PRICE[this.m_level + 1];
		}
		return null;
	}
		...
		if (this.m_active == true) {
			if (this.m_stunned <= 0) {
				...
				if (this.m_target == null) {
					rotateAmount = TURRET_GATLING_PATROL_SPEED[this.m_level] * deltaTime;
	...
	this.Shoot = function() {
		if (cooldownCount <= 0) {
			...
			battle.SpawnProjectile (PROJECTILE_GATLING, this.m_x, this.m_y, angle, this);
	...
	this.GetRange = function() {
		if (this.m_blinded > 0) {
			return TURRET_GATLING_RANGE[this.m_level] * BLIND_RANGE_MULTIPLIER;
		}
		else {
			return TURRET_GATLING_RANGE[this.m_level];
		}
	}
	this.GetRecoil = function() {
		if (this.m_blinded > 0) {
			return TURRET_GATLING_RECOIL[this.m_level] * BLIND_RECOIL_MULTIPLIER;
		}
		else {
			return TURRET_GATLING_RECOIL[this.m_level];
		}
	}
	this.GetRotateSpeed = function() {
		if (this.m_slowed > 0) {
			return TURRET_GATLING_ROTATE_SPEED[this.m_level] * SLOW_ROTATE_MULTIPLIER;
		}
		else {
			return TURRET_GATLING_ROTATE_SPEED[this.m_level];
		}
	}
	this.GetCooldown = function() {
		if (this.m_slowed > 0) {
			return TURRET_GATLING_COOLDOWN[this.m_level] * SLOW_COOLDOWN_MULTIPLIER;
		}
		else {
			return TURRET_GATLING_COOLDOWN[this.m_level];
		}
	}
