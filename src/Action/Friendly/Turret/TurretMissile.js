var TURRET_MISSILE_NAME = "Missile launcher";
var TURRET_MISSILE = 3;
var TURRET_MISSILE_PRICE = [250, 150, 175, 200, 225];
var TURRET_MISSILE_PATROL_SPEED = [30, 30, 30, 30, 30];
var TURRET_MISSILE_ROTATE_SPEED = [130, 135, 140, 145, 150];
var TURRET_MISSILE_RANGE = [12, 13, 14, 15, 16];
var TURRET_MISSILE_RECOIL = [3, 3, 3, 3, 3];
var TURRET_MISSILE_COOLDOWN = [3.5, 3.2, 2.9, 2.6, 2.3];

// Do nothing, just for display purpose
var TURRET_MISSILE_ACCURACY = [75, 78, 81, 84, 87];

function TurretMissile (battle, layer, x, y) {
	var SHADOW_OFFSET = 10;
	var PATROL_STAY_TIME = 5;
	
	// Properties
	this.m_active = true;
	this.m_type = TURRET_MISSILE;
	
	this.m_x = x;
	this.m_y = y;
	this.m_level = 0;
	this.m_price = TURRET_MISSILE_PRICE[0];
	...
	// Display info
	this.m_designPath = "res/GSAction/Turret/3-Missile/Design.png";
	this.m_name = TURRET_MISSILE_NAME;
	this.m_damageBar = [7, 7, 7, 8, 8];
	this.m_fireRateBar = [3, 4, 4, 5, 5];
	this.m_piercingBar = [1, 1, 1, 1, 1];
	this.m_rangeBar = [7, 7, 8, 8, 9];
	this.m_aoeBar = [4, 4, 5, 5, 6];
	
	this.MoneyRequireForNextLevel = function() {
		if (this.m_level < TURRET_MISSILE_PRICE.length - 1) {
			return TURRET_MISSILE_PRICE[this.m_level + 1];
		}
		return null;
	}
		...
		if (this.m_active == true) {
			if (this.m_stunned <= 0) {
				...
				if (this.m_target == null) {
					rotateAmount = TURRET_MISSILE_PATROL_SPEED[this.m_level] * deltaTime;
	...
	this.Shoot = function() {
		if (cooldownCount <= 0) {
			...
			battle.SpawnProjectile (PROJECTILE_MISSILE, this.m_x, this.m_y, angle, this.m_level, this.m_target);
	...
	this.GetRange = function() {
		if (this.m_blinded > 0) {
			return TURRET_MISSILE_RANGE[this.m_level] * BLIND_RANGE_MULTIPLIER;
		}
		else {
			return TURRET_MISSILE_RANGE[this.m_level];
		}
	}
	this.GetRecoil = function() {
		if (this.m_blinded > 0) {
			return TURRET_MISSILE_RECOIL[this.m_level] * BLIND_RECOIL_MULTIPLIER;
		}
		else {
			return TURRET_MISSILE_RECOIL[this.m_level];
		}
	}
	this.GetRotateSpeed = function() {
		if (this.m_slowed > 0) {
			return TURRET_MISSILE_ROTATE_SPEED[this.m_level] * SLOW_ROTATE_MULTIPLIER;
		}
		else {
			return TURRET_MISSILE_ROTATE_SPEED[this.m_level];
		}
	}
	this.GetCooldown = function() {
		if (this.m_slowed > 0) {
			return TURRET_MISSILE_COOLDOWN[this.m_level] * SLOW_COOLDOWN_MULTIPLIER;
		}
		else {
			return TURRET_MISSILE_COOLDOWN[this.m_level];
		}
	}
